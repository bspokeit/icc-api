...
class IccPatientXApi extends icc_api_1.IccPatientApi {
  ...
  static api(
    ...
    const hcPartyApi = new icc_hcparty_x_api_1.IccHcpartyXApi(host, headers, fetchImpl)
    ...
    return new IccPatientXApi(
      ...
      headers,
      cryptoApi,
      new icc_contact_x_api_1.IccContactXApi(host, headers, cryptoApi, fetchImpl),
      new icc_form_x_api_1.IccFormXApi(host, headers, cryptoApi, fetchImpl),
      new icc_helement_x_api_1.IccHelementXApi(host, headers, cryptoApi, fetchImpl),
      new icc_invoice_x_api_1.IccInvoiceXApi(
        host,
        headers,
        cryptoApi,
        new icc_api_1.IccEntityrefApi(host, headers, fetchImpl),
        fetchImpl
      ),
      new icc_document_x_api_1.IccDocumentXApi(
        host,
        headers,
        cryptoApi,
        new icc_api_1.IccAuthApi(host, headers, fetchImpl),
        fetchImpl
      ),
      hcPartyApi,
      new icc_classification_x_api_1.IccClassificationXApi(host, headers, cryptoApi, fetchImpl),
      new icc_calendar_item_x_api_1.IccCalendarItemXApi(host, headers, cryptoApi, fetchImpl),
      ["note"],
      fetchImpl
    )
  }
                        ...
                        utils_1.utils
                          ...
                          .then(p => {
                            if (p.picture && !(p.picture instanceof ArrayBuffer)) {
                              p.picture = ModelHelper_1.decodeBase64(p.picture)
                            }
                            return p
                          })
                      )
              ...
              return delSfks.length
                ? Promise.all([
                    ...
                    const oHes = hes.map(x =>
                      _.assign(new models_1.IcureStub({}), x, {
                    ...
                    const oIvs = ivs.map(x =>
                      _.assign(new models_1.Invoice({}), x, {
                        delegations: _.clone(x.delegations),
                        cryptedForeignKeys: _.clone(x.cryptedForeignKeys),
                        encryptionKeys: _.clone(x.encryptionKeys)
                      })
                    ...
                    const oCls = cls.map(x =>
                      _.assign(new models_1.Classification({}), x, {
                        delegations: _.clone(x.delegations),
                        cryptedForeignKeys: _.clone(x.cryptedForeignKeys),
                        encryptionKeys: _.clone(x.encryptionKeys)
                      })
                    ...
                    const oCis = cis.map(x =>
                      _.assign(new models_1.CalendarItem({}), x, {
                        delegations: _.clone(x.delegations),
                        cryptedForeignKeys: _.clone(x.cryptedForeignKeys),
                        encryptionKeys: _.clone(x.encryptionKeys)
                      })
                    ...
                    return net_utils_1
                      .retry(() =>
                        this.documentApi.getDocuments(
                          new models_1.ListOfIds({ ids: Object.keys(docIds) })
                        )
                      )
                      .then(docs => {
