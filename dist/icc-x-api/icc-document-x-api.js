...
class IccDocumentXApi extends icc_api_1.IccDocumentApi {
  constructor(
    ...
    this.utiRevDefs = Object.assign(Object.assign({}, this.compatUtiRevDefs), {
  ...
  initDelegationsAndEncryptionKeys(user, message, document) {
          ...
          .then(({ extractedKeys: sfks }) => {
            ...
            if (sfks.length && document.encryptedSelf) {
              return this.crypto.AES.importKey(
                  ...
                  key =>
                    new Promise(resolve => {
                      this.crypto.AES.decrypt(
                        key,
                        utils_1.utils.text2ua(atob(document.encryptedSelf))
                      ).then(resolve, () => {
                        console.log("Cannot decrypt document", document.id)
                        resolve(null)
                      })
                ...
                .then(decrypted => {
                  if (decrypted) {
                    document = _.extend(document, JSON.parse(utils_1.utils.ua2text(decrypted)))
                  }
                  return document
                })
            } else {
  ...
  getAttachmentAs(documentId, attachmentId, returnType, enckeys, fileName) {
    ...
    return XHR_1.XHR.sendCommand("GET", url, this.headers, null, this.fetchImpl, returnType)
      .then(doc => doc.body)
