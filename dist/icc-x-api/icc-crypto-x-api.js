...
class IccCryptoXApi {
  constructor(
    host,
    headers,
    hcpartyBaseApi, //Init with a hcparty x api for better performances
    patientBaseApi,
    ...
    this.hcpPreferenceKeyEhealthCert = "eHealthCRT"
    this.hcpartyBaseApi = hcpartyBaseApi
    this.patientBaseApi = patientBaseApi
    this.crypto = crypto
    this.storage = new storage_1.LocalStorageProxy(storage)
    this.generateKeyConcurrencyMap = {}
    this._AES = new AES_1.AESUtils(crypto)
    this._RSA = new RSA_1.RSAUtils(crypto, storage)
  ...
  forceGetHcPartyKeysForDelegate(delegateHcPartyId) {
    return Promise.all([
  ...
  randomUuid() {
    return ((1e7).toString() + -1e3 + -4e3 + -8e3 + -1e11).replace(
      ...
      c =>
        (
          Number(c) ^
          (this.crypto.getRandomValues(new Uint8Array(1))[0] & (15 >> (Number(c) / 4)))
        ).toString(16) //Keep that inlined or you will loose the random
    )
  }
  encryptedShamirRSAKey(hcp, notaries, threshold) {
    return this._RSA.loadKeyPairImported(hcp.id).then(keyPair =>
      this._RSA.exportKey(keyPair.privateKey, "pkcs8").then(exportedKey => {
        ...
        return _.reduce(
          notaries,
          (queue, notary, idx) => {
            return queue.then(hcp =>
              __awaiter(this, void 0, void 0, function*() {
                const hcParty = hcp.hcPartyKeys[notary.id]
                  ? hcp
                  : yield this.generateKeyForDelegate(hcp.id, notary.id)
                try {
                  ...
                  const encryptedShamirPartition = yield this.AES.encrypt(
  ...
  /* Reconstructs the hcp's private key from the notaries' shamir shares and stores it in localstorage.
    ...
    **/
  decryptedShamirRSAKey(hcp, notaries) {
      ...
      try {
        ...
        if (nLen == 1) {
          ...
          decryptedPrivatedKey = this.utils.ua2hex(
            yield this.AES.decrypt(importedAESHcPartyKey.key, this.utils.hex2ua(cryptedPrivatedKey))
          )
        } else {
          const decryptedShares = yield _.reduce(
            notaries,
            (queue, notary) => {
              return queue.then(shares =>
                __awaiter(this, void 0, void 0, function*() {
                  try {
        ...
        const importedPrivateKey = yield this.RSA.importKey(
          "pkcs8",
          this.utils.hex2ua(decryptedPrivatedKey),
          ["decrypt"]
        )
        const importedPublicKey = yield this.RSA.importKey(
          "spki",
          this.utils.hex2ua(hcp.publicKey),
          ["encrypt"]
        )
        const exportedKeyPair = yield this.RSA.exportKeys(
   ...
   */
  cacheKeyPair(keyPairInJwk, hcPartyKeyOwner) {
    if (!keyPairInJwk) {
  ...
  decryptAndImportAesHcPartyKeysForDelegators(
    ...
    return (
    ...
    ).then(delegatorIDsWithDelegateEncryptedHcPartyKey => {
      ...
      return Promise.all(
        delegatorsHcPartyIdsSet.map(delegatorId => {
      ...
      ).then(hcPartyKeys => hcPartyKeys.filter(hcPartyKey => !!hcPartyKey))
    })
  }
  /**
    ...
    return this.getHcpOrPatient(ownerId)
      ...
      .then(importedAESHcPartyKey =>
        Promise.all([
          this._AES.encrypt(
            importedAESHcPartyKey.key,
            utils_1.utils.text2ua(createdObject.id + ":" + secretId).buffer,
            importedAESHcPartyKey.rawKey
          ),
          parentObject
            ? this._AES.encrypt(
                importedAESHcPartyKey.key,
                utils_1.utils.text2ua(createdObject.id + ":" + parentObject.id).buffer,
                importedAESHcPartyKey.rawKey
              )
    ...
    return this.getHcpOrPatient(ownerId)
      ...
      .then(importedAESHcPartyKey =>
        Promise.all([
          ...
          this._AES.encrypt(
            importedAESHcPartyKey.key,
            utils_1.utils.text2ua(modifiedObject.id + ":" + secretIdOfModifiedObject).buffer,
            importedAESHcPartyKey.rawKey
          ),
          parentObject
            ? this._AES.encrypt(
                importedAESHcPartyKey.key,
                utils_1.utils.text2ua(modifiedObject.id + ":" + parentObject.id).buffer,
                importedAESHcPartyKey.rawKey
              )
            : Promise.resolve(null)
        ])
      ...
      .then(
        ([
          ...
          const delegationCryptedDecrypted = _.merge(
            ((modifiedObject.delegations || {})[delegateId] || []).map(d => ({
              d
            })),
            (previousDecryptedDelegations || [])
          ...
          const cryptedForeignKeysCryptedDecrypted = _.merge(
            ((modifiedObject.cryptedForeignKeys || {})[delegateId] || []).map(d => ({
              d
            })),
            (previousDecryptedCryptedForeignKeys || [])
              .map(dd => (dd ? this._utils.ua2text(dd) : null))
  ...
  initEncryptionKeys(createdObject, ownerId) {
    ...
    return this.getHcPartyKeysForDelegate(ownerId)
      ...
      .then(importedAESHcPartyKey =>
        this._AES.encrypt(
          importedAESHcPartyKey.key,
          utils_1.utils.text2ua(createdObject.id + ":" + secretId)
        )
      )
    ...
    return this.getHcpOrPatient(ownerId)
      ...
      .then(importedAESHcPartyKey =>
        Promise.all([
          ...
          this._AES.encrypt(
            importedAESHcPartyKey.key,
            utils_1.utils.text2ua(modifiedObject.id + ":" + secretEncryptionKeyOfObject)
          )
        ])
      )
      .then(([previousDecryptedEncryptionKeys, encryptedEncryptionKey]) => {
        ...
        const encryptionKeysCryptedDecrypted = _.merge(
          ((modifiedObject.encryptionKeys || {})[delegateId] || []).map(d => ({
            d
          })),
        ...
        allEncryptionKeys[delegateId] = _.uniqBy(encryptionKeysCryptedDecrypted, x => x.k).map(
          x => x.d
        )
        return {
            ...
            delegations: _.assignWith(
              ...
              (dest, src) =>
                (src || []).concat(
                  _.filter(
                    dest,
                    d => !src.some(s => s.owner === d.owner && s.delegatedTo === d.delegatedTo)
                  )
                )
            ),
        ...
        decryptPromises.push(
          this._AES
            ...
            .then(decryptedGenericDelegationKey => {
              const results = utils_1.utils.ua2text(decryptedGenericDelegationKey).split(":")
    ...
    return this.getHcpOrPatient(healthcarePartyId)
      ...
      .then(publicKey => {
        ...
        return this._RSA.importKeyPair(
          ...
          "jwk",
          utils_1.utils.spkiToJwk(utils_1.utils.hex2ua(publicKey))
        )
      })
      .then(keyPair => {
  ...
  loadKeyPairsAsJwkInBrowserLocalStorage(healthcarePartyId, privKey) {
    return this.hcpartyBaseApi
      ...
      .then(publicKey => {
        const pubKey = utils_1.utils.spkiToJwk(utils_1.utils.hex2ua(publicKey.hexString))
        privKey.n = pubKey.n
        privKey.e = pubKey.e
  ...
  loadKeyPairsInBrowserLocalStorage(healthcarePartyId, file) {
    ...
    return new Promise((resolve, reject) => {
      ...
      fr.onload = e => {
        ...
        this.loadKeyPairsAsTextInBrowserLocalStorage(
          healthcarePartyId,
          utils_1.utils.hex2ua(privateKey)
        )
          .then(resolve)
          .catch(reject)
      }
    ...
    return this.utils.notConcurrent(this.generateKeyConcurrencyMap, ownerId, () =>
      Promise.all([this.getHcpOrPatient(ownerId), this.getHcpOrPatient(delegateId)]).then(
        ([owner, delegate]) => {
          ...
          const ownerType =
            owner.constructor && owner.constructor.name === "Patient" ? "patient" : "hcp"
          const genProm = new Promise((resolve, reject) => {
            delegate.publicKey
              ? this._AES
                  .generateCryptoKey(true)
                  .then(AESKey => {
                    const ownerPubKey = utils_1.utils.spkiToJwk(
                      utils_1.utils.hex2ua(owner.publicKey)
                    )
                    const delegatePubKey = utils_1.utils.spkiToJwk(
                      utils_1.utils.hex2ua(delegate.publicKey)
                    )
                    return Promise.all([
                      ...
                      Promise.all([
                        this._RSA.encrypt(ownerImportedKey, this._utils.hex2ua(AESKey)),
                        this._RSA.encrypt(delegateImportedKey, this._utils.hex2ua(AESKey))
                      ])
                    )
                  })
                  .then(
                  ...
                  .then(() => {
                    ownerType === "hcp"
                      ? this.hcpartyBaseApi
                          ...
                          .then(hcp => resolve(["hcp", hcp]))
                      : this.patientBaseApi
                          .modifyPatient(owner)
                          .then(pat => resolve(["patient", pat]))
                  })
                  .catch(e => reject(e))
  ...
  checkPrivateKeyValidity(hcp) {
    return new Promise(resolve => {
      this._RSA
        .importKey("jwk", utils_1.utils.spkiToJwk(utils_1.utils.hex2ua(hcp.publicKey)), ["encrypt"])
